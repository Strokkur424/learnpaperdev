...
<details>
  ...
  <FileTree>
    - BeginnerPlugin - gradle/ - src - main - java - your - package - name -
    BeginnerPlugin.java - build.gradle.kts - gradlew - gradlew.bat -
    settings.gradle.kts
  </FileTree>
</details>
...
First, we will fill up our `BeginnerPlugin.java` file with a bit of logic. Every Paper plugin needs to have a class with extends the `JavaPlugin` class. It is furthermore considered
good practice to make your **plugin main class** (this is what you call the class that extends `JavaPlugin`) `final` to avoid any intended or unintended extending.
...
With out main class sorted out, there is still one thing left to do: Adding a `paper-plugin.yml`.
Each Paper plugin needs to have either a `plugin.yml` or `paper-plugin.yml` file which declares the plugin's name, when it should be loaded, where the main class is located, and other metadata.

For this, first create a new folder, `src/main/resources`, and then create a new file, `src/main/resources/paper-plugin.yml`.

<FileTree>
  - main/java - com/learnpaperdev/beginner - BeginnerPlugin.java - resources -
  paper-plugin.yml
</FileTree>
...
<Aside title="Why are we using paper-plugin.yml and not plugin.yml?" type="tip">
    ...
    our usage of Paper API. So this also acts as a way to block non-Paper servers from loading our plugin without us needing to implement any additional, potentially ugly checks.
...
<Aside title="Java 21" type="caution">
    ...
    If your Java version is below 21, or you somehow managed to get this far without any Java installation at all, you should following the Paper guide on installing/updating Java: https://docs.papermc.io/misc/java-install.
</Aside>
...
<Code code={StartLog} lang="ansi" showLineNumbers="false" collapse="[0-0, 9-27, 30-41, 44-48, 54-85]"/>
